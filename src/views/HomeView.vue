	...
	<main class="py-2">
		...
		<div class="flex flex-col gap-4 mt-4">
			<button @click="quizStore.loadEasyQuestions" :disabled="quizStore.loading" class="btn btn-primary btn-lg text-xl">
				<span v-if="quizStore.loading && quizStore.level === QuestionLevel.Easy" class="loading loading-spinner"></span>
				Easy
			</button>
			<button :disabled="quizStore.loading" class="btn btn-primary btn-lg text-xl">
				<span v-if="quizStore.loading && quizStore.level === QuestionLevel.Medium" class="loading loading-spinner"></span>
				Medium
			</button>
			<button :disabled="quizStore.loading" class="btn btn-primary btn-lg text-xl">
				<span v-if="quizStore.loading && quizStore.level === QuestionLevel.Hard" class="loading loading-spinner"></span>
				Hard
			</button>
			<div class="flex justify-around mt-8 md:mt-20">
				...
				<RouterLink :to="{ name: 'questions' }" class="btn btn-neutral btn-sm btn-ghost">
					<IconCheck />Rate
				</RouterLink>
			</div>
		</div>
